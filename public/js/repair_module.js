$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        var unit_purchase_price = __read_number(tr.find('input.unit_purchase_price'));
        
        var line_total = entered_qty * unit_price_inc_tax;
        var line_purchase = entered_qty * unit_purchase_price;
        __write_number(tr.find('input.pos_line_total'), line_total, false, 2);
        __write_number(tr.find('input.pos_line_purchase_text'), unit_purchase_price, false, 2);
        
        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
        tr.find('span.pos_line_purchase_text').text(__currency_trans_from_en(line_purchase, true));
...
function get_product_suggestion_list(category_id,term, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null) {
                ...
                if (result.success) {
                    ...
                    if (result.html_modifier) {
                        $('table#pos_table tbody')
                            ...
                            .last()
                            .find('td:first')
                            .prepend(result.html_modifier);
                    }

                    //scroll bottom of items list
                    $(".pos_product_div").animate({ scrollTop:0}, 1000);
                } else {
                    toastr.error(result.msg);
                    $('input#search_product')
                        .focus()
