...
//                $product['products'][$key]['unit_price_inc_tax'] = $product['unit_price_inc_tax_new'] / 1.15;
//                $product['unit_price'] = $product['products'][$key]['unit_price_inc_tax'];
//            }
//            dd($product['unit_price']);
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                /* changed by eng mohamed ali ot add payment */
                /*if ($payment_amount != 0)*/
                if ($payment_amount > -1) {
                    $prefix_type = 'sell_payment';
                    if ($transaction->type == 'purchase') {
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output = [
            ...
            /*eng ali 25-11-2021*/
            'tax_number' =>__('lang_v1.tax_number').' : '. trim($il->tax_number),
            'tax_number2' =>trim($il->tax_number2),
            'commercial_register' =>__('lang_v1.commercial_register').' : '. trim($il->commercial_register),
            /* End*/
        ...
        if ($il->show_customer == 1) {
            ...
            if (!empty($output['customer_name']) && $receipt_printer_type != 'printer') {
                ...
                if (!empty($customer->landline)) {
                    ...
                    $output['mobile'] .= ' - ' . $customer->landline;
                }
                if (!empty($customer->alternate_number)) {
                    $output['mobile'] .= ' - ' . $customer->alternate_number;
                }
            }
        ...
        /*dd($transaction->status .' : '.$transaction->is_quotation .' : ' .$transaction->sub_status);*/
        /*this code add by eng mohamed ali to change  label of invoice*/

        /* عرض أسعار*/
        if($transaction->status == 'draft' && $transaction->is_quotation == 1){
            $output['invoice_heading']=__('lang_v1.quotation');
        }

        if($transaction->status == 'draft'  && is_null($transaction->sub_status) && $transaction->is_quotation == 0){
            $output['invoice_heading']=__('sale.draft');
        }

        if($transaction->status == 'draft' && $transaction->sub_status=='proforma'){
            $output['invoice_heading']=__('lang_v1.proforma');
        }
        /* dd( $output['invoice_heading']);*/
        ...
        $output['total_discount']=$this->num_f(($total_line_discount+$discount), true, $business_details);
...
//                            dd(0);
        ...
        // edit by eng ali json_decode for larvel 8
        $qr_code_fields = !empty($il->qr_code_fields) ? json_decode($il->qr_code_fields) : [];



        if (in_array('business_name', $qr_code_fields)) {
            $qr_code_details[] = __('business.business') . ': ' . $business_details->name;
        }
        if (in_array('address', $qr_code_fields)) {
            $qr_code_details[] = __('business.address') . ': ' .$output['address'];
        }
        if (in_array('tax_1', $qr_code_fields)) {
            $qr_code_details[] = $business_details->tax_label_1 . ': ' . $business_details->tax_number_1;
        }
        if (in_array('tax_2', $qr_code_fields)) {
            $qr_code_details[] = $business_details->tax_label_2 . ' ' . $business_details->tax_number_2;
        }
        if (in_array('invoice_no', $qr_code_fields)) {
            $qr_code_details[] = $il->invoice_no_prefix . ': ' . $transaction->invoice_no;
        }
        if (in_array('invoice_datetime', $qr_code_fields)) {
            $qr_code_details[] = $output['date_label'] . ': ' . $output['invoice_date'];
        }
        if (in_array('subtotal', $qr_code_fields)) {
            $qr_code_details[] = $output['subtotal_label'] . ' ' . $output['subtotal'];
        }
        if (in_array('total_amount', $qr_code_fields)) {
            $qr_code_details[] = $output['total_label'] . ' ' . $output['total'];
        }
        if (in_array('total_tax', $qr_code_fields)) {
            $total_order_tax = $transaction->tax_amount + $total_line_taxes;
            $qr_code_details[] = __('sale.tax') . ': ' . $this->num_f($total_order_tax, $show_currency, $business_details);
        }
        if (in_array('customer_name', $qr_code_fields)) {
            $cust_label = $il->customer_label ?? __('contact.customer');
            $qr_code_details[] = $cust_label . ': ' . $customer->full_name;
        }
        ...
        $tax_info1=$business_details->tax_number_1;
        $l2=dechex(strlen($tax_info1));
        $l2=strlen($l2)==1? '0'.$l2:$l2;
        $encoded2 ='02'.$l2. bin2hex($tax_info1);

        $transaction_date=Carbon::parse($transaction->transaction_date)->format('Y-m-d')."T" .Carbon::parse($transaction->transaction_date)->format('H-i-s')."Z";
        $encoded3 ='0314'. bin2hex($transaction_date);


        $total_final=$transaction->final_total;
        $l4=dechex(strlen($total_final));
        $l4=strlen($l4)==1? '0'.$l4:$l4;
        $encoded4 ='04'.$l4. bin2hex($total_final);

        /* Tax amount after dicount */
        $vat_total=$transaction->tax_amount; // 6
        ...
        $qr_code = base64_encode(pack('H*',$encoded));
        $output['qr_code_gen']=$qr_code;


        /* New Qr-Code eng ali */
        $location_details = BusinessLocation::find($location_id);
//dd($location_details->custom_field2);
        $total_order_tax = $transaction->tax_amount + $total_line_taxes;
//        $qr_code_text = $this->_zatca_qr_text($business_details->name, $business_details->tax_number_1, $transaction->transaction_date, $transaction->final_total, $total_order_tax);
        ...
        $output['qr_code_gen']=$qr_code_text;
...
//        dd($curr_total_payment);
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        // added by eng mohamed ali for client payment 11-12-2021
        $total_final=$total_debit-$total_credit;
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        $data['net_profit'] = $module_total + $gross_profit
            ...
            ) - ( $data['total_reward_amount'] + $data['total_expense'] + $data['total_adjustment'] + $data['total_transfer_shipping_charges'] + $data['total_purchase_shipping_charge'] + $data['total_purchase_additional_expense']
            );
        ...
        $data['installmentprofit']=$this->getinstallmentprofit($business_id,$location_id);

        return $data;
    }

    public function getinstallmentprofit($business_id, $location_id ){
        ...
        if(Module::has('Installment')) {
            $installment = DB::table('installments')
                ...
                ->where('business_id','=',$business_id)
                ->sum('benefit_value');
            $installmentprofit=$installment;
        }

        return $installmentprofit;
    }
...
//dd($sell_return);
    ...
    public function gettransaction($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->select(
                DB::raw('SUM(total_before_tax) as total_before_tax'),
                DB::raw("SUM(IF(discount_type = 'percentage', COALESCE(discount_amount, 0)*total_before_tax/100, COALESCE(discount_amount, 0) )) as discount_amount"),
                DB::raw('SUM(tax_amount) as tax_amount'),
                DB::raw('SUM(shipping_charges) as shipping_charges'),
                DB::raw('SUM(packing_charge) as packing_charge'),
                DB::raw('SUM(round_off_amount) as round_off_amount'),
    ...
    public function getselreturn($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
