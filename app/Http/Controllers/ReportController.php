        ...
        if ($request->ajax()) {
            ...
            $datatable =  Datatables::of($products)
                ...
                ->editColumn('default_purchase_price', function ($row) {
                    ...
                    return  '<span data-is_quantity="true" class="current_stock display_currency" data-orig-value="' . (float)$row->default_purchase_price . '" data-unit="' . $row->default_purchase_price . '" data-currency_symbol=false > ' . (float)$row->default_purchase_price . '</span>' . ' '  ;
                ...
                ->editColumn('stock_price', function ($row) {
                    $curr_rate= request()->session()->get('business.rate_currency')? request()->session()->get('business.rate_currency'): 0;
                ...
                ->addColumn('stock_price_curr', function ($row) {
                    $curr_rate= request()->session()->get('business.rate_currency')? request()->session()->get('business.rate_currency'): 1;
                    $second_currency_id = request()->session()->get('business.second_currency_id');;
                    $second_currency=\DB::table('currencies')->where('id',$second_currency_id)->first();
                    if($second_currency){
                        $curr_code= $second_currency->code;
                    }else{
                        $curr_code='';
                    }
                    $purchaseIn_curr=(float)$row->stock_price / (float)$curr_rate;
                    $html = '<span class="total_stock_price_curr"'
                        . (float)$row->stock_price / (float)$curr_rate. '">'
                        . number_format($purchaseIn_curr, 2, '.', '').''.$curr_code.'</span>';
                ...
                ->addColumn('stock_value_by_sale_price_curr', function ($row) {
                    $curr_rate= request()->session()->get('business.rate_currency')? request()->session()->get('business.rate_currency'): 1;
                    $second_currency_id = request()->session()->get('business.second_currency_id');;
                    $second_currency=\DB::table('currencies')->where('id',$second_currency_id)->first();
                    if($second_currency){
                    ...
                    $unit_selling_price = (float)$row->group_price > 0 ? $row->group_price : $row->unit_price;
                    $stock_price = $stock * $unit_selling_price;
                    return  '<span class="stock_value_by_sale_price " data-orig-value="' . (float)$stock_price / (float)$curr_rate. '" data-currency_symbol=true > ' . number_format((float)$stock_price /(float)$curr_rate, 2, '.', ''). '   '.$curr_code.'</span>';
                })
            ...
            $raw_columns  = ['unit_price', 'total_transfered', 'total_sold','default_purchase_price','stock_price_curr',
                'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price', 'stock_value_by_sale_price_curr','potential_profit'];
...
//dd($type);
            ...
            foreach ($taxes as $tax) {
                ...
                $datatable->addColumn($col, function($row) use($tax, $type, $col, $group_taxes) {
                    ...
                    if ($row->tax_id == $tax['id']) {
                        $return_tax_amount=\DB::table('transactions')->where('return_parent_id',$row->id)->sum('tax_amount');
                        $tax_amount += $row->tax_amount - ($return_tax_amount);
                    }
    ...
    public function getLessTrendingProducts(Request $request)
    {
        if (!auth()->user()->can('less_trending_product_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $products = $this->productUtil->getLessTrendingProducts($business_id, $filters);
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('trans_id', function ($row) {
                    return '<a  class="btn btn-primary" href="'.action('PurchaseReturnController@add', [$row->transaction_id]).'" >مرتجع</a>';
                })
                ->editColumn('ref_no', function ($row) {
                ...
                ->rawColumns(['trans_id','ref_no', 'unit_purchase_price', 'subtotal', 'purchase_qty', 'quantity_adjusted', 'supplier'])
                ->make(true);
        }
...
//                    'transaction_sell_lines.unit_price_inc_tax  as unit_sale_price',
                    DB::raw('((transaction_sell_lines.unit_price_inc_tax)) + (t.tax_amount/transaction_sell_lines.quantity) as unit_sale_price'),
                    ...
                    'u.short_name as unit',
                    DB::raw('(CASE 
                        WHEN ((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) != 0
                        THEN ((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) + t.tax_amount 
                        ELSE ((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax)
                        END) as subtotal'),
                )
            ...
            return Datatables::of($query)
                ->editColumn('trans_id', function ($row) {
                    return '<a  class="btn btn-primary" href="'.action('SellReturnController@add', [$row->transaction_id]).'" >مرتجع</a>';
                })
...
//                       '</span>'.'<br>'.'<span class="tax" data-orig-value="'.(float)$row->item_tax.'" data-unit="'.$row->tax.'"><small>('.$row->tax.')</small></span>';
                    return '<span class="display_currency" data-currency_symbol = true>'.
                        $row->tax_amount.
                        '</span>'.'<br>'.'<span class="tax" data-orig-value="'.(float)$row->tax_amount.'" data-unit="'.$row->tax.'"><small>('.$row->tax.')</small></span>';
                })
                ->editColumn('customer', '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$customer}}')
                ->rawColumns(['trans_id','invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax', 'customer'])
                ->make(true);
        }
...
//sell for return
    public function getproductSellReturnReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->rawColumns(['trans_id','invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax'])
                ->make(true);
        }
    ...
    public function itemsReport()
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($query)
                ...
                // $stock= \DB::select("select sum(qty_available) as current_stock from variation_location_details where variation_id=$row->variation_id AND location_id = $row->location_id");
                // return  0;
                //$stock[0]->current_stock ;
                // })
    ...
    public function sell_purchase_lines_report(){
        if (!auth()->user()->can('sell_purchase_lines_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        $business_id = request()->session()->get('user.business_id');

        $products=\DB::table('products')->where('business_id',$business_id)->orderBy('id','desc')->select('id','name')->limit(200)->get();
    ...
    public function get_product_trans(Request $request){
        ...
        $purchases=\DB::table('purchase_lines as PL')
            ->join('transactions as tr','PL.transaction_id','tr.id')
            ->leftjoin('contacts','tr.contact_id','contacts.id')
            ->select(
                'PL.transaction_id',
                'tr.ref_no',
                'PL.created_at',
        ...
        $sells=\DB::table('transaction_sell_lines as SL')
            ->join('transactions as tr','SL.transaction_id','tr.id')
            ->join('contacts','tr.contact_id','contacts.id')
            ->select(
                'SL.transaction_id',
                'tr.invoice_no as ref_no',
                'SL.created_at',
                'SL.quantity',
        ...
        $stocktackings=\DB::table('stocktacking_lines as SL')
            ->join('transactions as tr','SL.transaction_id','tr.id')
            ->join('variations','SL.variation_id','variations.id')
            ->join('products','products.id','variations.product_id')
            ->select(
                'SL.transaction_id',

                'SL.created_at',
                'SL.real_qty_available as quantity',
                'SL.current_stock',
            ...
            ->where('SL.variation_id',$variation_id)->get();
        ...
        foreach($purchases as $row){
            if($row->quantity_returned>0){
                $table[$index]['created_at']=$row->created_at;
                $table[$index]['op']='شراء';
                if($row->type=='opening_stock'){
                    $table[$index]['op']='مخزون افتتاحي';
                }
                $url=\URL::to("/purchases/$row->transaction_id");
                $table[$index]['ref_no']='<a href="'.$url.'">'.$row->ref_no.'</a>';;
                $table[$index]['contact']=$row->name;
                $table[$index]['in']=$row->quantity;
                $table[$index]['out']='';
                $c_qty=\DB::table('variation_location_details')->where('variation_id',$variation_id)->where('created_at','<=',$row->created_at)->SUM('qty_available');
                $table[$index]['current_stock']=$c_qty;
                $index++;
                $table[$index]['created_at']=$row->created_at;
                $table[$index]['op']='مرتجع شراء';
                $table[$index]['ref_no']=$row->ref_no;
                $table[$index]['contact']=$row->name;
                $table[$index]['in']='';
                $table[$index]['out']=$row->quantity_returned;
                $c_qty=\DB::table('variation_location_details')->where('variation_id',$variation_id)->where('created_at','<=',$row->created_at)->SUM('qty_available');
                $table[$index]['current_stock']=$c_qty;
                $index++;
            }else{
        ...
        foreach($sells as $row){
            if($row->quantity_returned>0){
                ...
                $table[$index]['contact']=$row->name;
                $table[$index]['in']='';
                $table[$index]['out']=$row->quantity;;
                $c_qty=\DB::table('variation_location_details')->where('variation_id',$variation_id)->where('created_at','<=',$row->created_at)->SUM('qty_available');
                $table[$index]['current_stock']=$c_qty;

                $index++;
                $table[$index]['created_at']=$row->created_at;
                $table[$index]['op']='مرتجع بيع';
                $table[$index]['ref_no']=$row->ref_no;
                $table[$index]['contact']=$row->name;
                $table[$index]['in']=$row->quantity_returned;
                $table[$index]['out']="";
                $c_qty=\DB::table('variation_location_details')->where('variation_id',$variation_id)->where('created_at','<=',$row->created_at)->SUM('qty_available');
                $table[$index]['current_stock']=$c_qty;
                $index++;
        ...
        //stocktacking
        foreach($stocktackings as $row){

            $table[$index]['created_at']=$row->created_at;
            $table[$index]['op']='جرد مخازن';

            $url=\URL::to("/stocktacking/report/$row->transaction_id");
            $table[$index]['ref_no']='<a href="'.$url.'">'.$row->transaction_id.'</a>';;
            $table[$index]['contact']='';
            $diff_qty=$row->quantity - $row->current_stock;
            if($diff_qty>=0){
                $table[$index]['in']=$diff_qty;
                $table[$index]['out']='';
            }else{
                $table[$index]['in']='';
                $table[$index]['out']=$diff_qty;
            }

            $c_qty=\DB::table('variation_location_details')->where('variation_id',$variation_id)->SUM('qty_available');
            $table[$index]['current_stock']=$c_qty;
            $index++;
    ...
    public function compare_locations(Request $request)
    {
        $location_id2=0;

        if (!auth()->user()->can('stcok_compares')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()&&request()->only('location_id')!=="") {

            $location_id2= request()->only('location_id2');
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type',
                'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state']);

            $filters['not_for_selling'] = isset($filters['not_for_selling']) && $filters['not_for_selling'] == 'true' ? 1 : 0;

            $filters[''] = $show_manufacturing_data;
            ...
            $datatable =  Datatables::of($products)
                ->addColumn('action', function ($row) {
                    $html='<input class="form-control" type="text" name="id" value="'.$row->id.'" />';
                    return $html;
                })
                ->editColumn('stock', function ($row) {
                    if ($row->enable_stock) {
                ...
                ->addColumn('stock2', function ($row) {
                    $location_id2=request()->input('location_id2') ;
                    $stock2= \DB::select("select sum(qty_available) as current_stock from variation_location_details where variation_id=$row->variation_id AND location_id = $location_id2");
                    $stockQ2=$stock2[0]->current_stock ? $stock2[0]->current_stock  : 0;
                    return  (float)$stockQ2.' '. $row->unit ;
                })
            ...
            $raw_columns  = ['action','unit_price', 'total_transfered', 'total_sold',
...
//dd($business_locations);
    ...
    // Missing amounts report
    public function getStockReportMissing(Request $request)
    {
        if (!auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $for = request()->input('for') == 'view_product' ? 'view_product' :'datatables';
            $alert_qty=true;
            $products = $this->productUtil->getProductStockDetails($business_id, $filters, $for,$alert_qty);
            ...
            $datatable =  Datatables::of($products)
                ...
                ->addColumn('alert_qty', function ($row) {
                    if ($row->enable_stock) {
                        $qty = $row->alert_quantity ? $row->alert_quantity : 0 ;
                        return  '<span data-is_quantity="true" class="current_stock display_currency" data-orig-value="' . (float)$qty . '" data-unit="' . $row->unit . '" data-currency_symbol=false > ' . (float)$qty . '</span>' . ' ' . $row->unit ;
                    } else {
                        return 'N/A';
                    }
                })
                ...
                ->addColumn('missing_qty', function ($row) {
                    if ($row->enable_stock) {
                        $missing=$row->alert_quantity - $row->stock;
                        $qty =  $missing > 0 ? $missing : 0 ;
                        return  '<span data-is_quantity="true" class="current_stock display_currency" data-orig-value="' . (float)$qty . '" data-unit="' . $row->unit . '" data-currency_symbol=false > ' . (float)$qty . '</span>' . ' ' . $row->unit ;
                    } else {
                        return 'N/A';
                    }
                ...
                ->removeColumn('unit')
                ->removeColumn('id');
            $raw_columns  = ['missing_qty','alert_qty','unit_price', 'total_transfered', 'total_sold','default_purchase_price',
                'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price', 'potential_profit'];
            if ($show_manufacturing_data) {
                $datatable->editColumn('total_mfg_stock', function ($row) {
                    $total_mfg_stock = 0;
    ...
    public function getsells(Request $request)
    {
        ...
        if($request->ajax()) {
            ...
            //$sell_details = $this->transactionUtil->getSellTotals($business_id, $start_date, $end_date, $location_id, $created_by);
            $sell_details = $this->transactionUtil->gettransaction($business_id, $start_date, $end_date, $location_id, $created_by);
            $sell_return = $this->transactionUtil->getselreturn($business_id, $start_date, $end_date, $location_id, $created_by);

            $totall_sell = $sell_details->total_before_tax + $sell_details->tax_amount + $sell_details->shipping_charges - $sell_details->discount_amount - $sell_details->round_off_amount + $sell_details->packing_charge;
            $totall_return = $sell_return->total_before_tax + $sell_return->tax_amount + $sell_return->shipping_charges - $sell_return->discount_amount - $sell_return->round_off_amount + $sell_return->packing_charge;

            $output=view('report.partials.sellsall',compact('sell_details','sell_return','totall_sell','totall_return'));
            /*dd($sell_return);
            ,'sell_details','sell_return','totall_sell','totall_return'
            */
