...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        /*  dd(session('business.id'), auth()->user()->hasRole('Admin#' . session('business.id')));*/
            ...
            //Stock transfer dropdown 35
            if (in_array('stock_transfers', $enabled_modules) && (auth()->user()->can('stocktacking.view') || auth()->user()->can('stocktacking.create'))) {
                $menu->dropdown(
                    __('جرد المخازن'),
                    function ($sub) {
                        ...
                        if (auth()->user()->can('stocktacking.view')) {
                            $sub->url( action('StocktackingController@index'),
                                        __('جرد المخازن'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stocktacking']
                            );
                        }
