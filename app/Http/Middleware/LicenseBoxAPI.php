...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '6F3BA184';
		$this->api_url = 'https://licensebox.Azha.com/';
		$this->api_key = 'D387CEED9766B0117BB6';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["7d1d6e87285dfb8"])){
				$_SESSION["7d1d6e87285dfb8"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["7d1d6e87285dfb8"])){
				$get_data = $this->call_api(
					'POST',
