                ...
                ->addColumn('action', function($row) {
                    ...
                    if (auth()->user()->can("job_sheet.view_assigned") || auth()->user()->can("job_sheet.view_all") || auth()->user()->can("job_sheet.create")) {
                            $html .= '<li>
                                ...
                                <li>
                                <a href="' . action('\Modules\Repair\Http\Controllers\JobSheetController@print_slim', ['id' => $row->id]) . '" class="cursor-pointer">
                    ...
                    if ($row->invoices->count() > 0) {
                        ...
                        if($transaction !==NULL){
                              ...
                              $html .= '<li><a href="' . action('TransactionPaymentController@addPayment', [$transaction->id]) . '" class="add_payment_modal"><i class="fas fa-money-bill-alt"></i> ' . __("purchase.add_payment") . '</a></li>';
...
*/
                ...
                ->addColumn('final_total',function($row){
                      ...
                      if ($row->invoices->count() > 0) {
                        foreach ($row->invoices as $key => $invoice) {
                            $invoice_no[] = $invoice->invoice_no;
                            $transaction=\DB::table('transactions')->where('invoice_no',$invoice->invoice_no)->where('business_id',$business_id)->first();
                        }
                        return '<span class="display_currency total-discount" data-currency_symbol="true" data-orig-value="'.$transaction->final_total.'">'.$transaction->final_total.'</span> ';
                    }  
                })
                ->addColumn('total_paid',function($row){
                      ...
                      if ($row->invoices->count() > 0) {
                        foreach ($row->invoices as $key => $invoice) {
                            $invoice_no[] = $invoice->invoice_no;
                            $transaction=\DB::table('transactions')->where('invoice_no',$invoice->invoice_no)->where('business_id',$business_id)->first();
                            $total_paid=\DB::table('transaction_payments')->where('transaction_id',$transaction->id)->sum('amount');
                        }
                        return '<span class="display_currency total-discount" data-currency_symbol="true" data-orig-value="'.$total_paid.'">'.$total_paid.'</span> ';
                    }  
                })
                ->addColumn('total_due',function($row){
                      ...
                      if ($row->invoices->count() > 0) {
                        ...
                        $total_due=$transaction->final_total - $total_paid;
                        return '<span class="display_currency total-discount" data-currency_symbol="true" data-orig-value="'.$total_due.'">'.$total_due.'</span> ';
                    }  
                })
                ->removeColumn('id')
                ->rawColumns(['action', 'final_total','total_paid','total_due','service_type', 'delivery_date', 'repair_no', 'status', 'estimated_cost', 'created_at'])
                ->make(true);
        }
