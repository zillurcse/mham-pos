...
class SuperadminSubscriptionsController extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('superadmin::'.viewSource().'superadmin_subscription.index');
    }
    ...
    public function create()
    {
        ...
        return view('superadmin::'.viewSource().'superadmin_subscription.add_subscription')
    ...
    public function edit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('superadmin::'.viewSource().'superadmin_subscription.edit')
    ...
    public function update(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            try {
                $business_id = $request->session()->get('user.business_id');//set as error by eng mohamed ali
                ...
                if ($validateNumber && $business_location->mobile != null && $subscriptions->start_date != null && $subscriptions->end_date != null) {
                    $startDate = Carbon::parse($subscriptions->start_date)->format('Y-m-d');
//                    $endDate = Carbon::parse($subscriptions->end_date)->format('Y-m-d');
//                    $trialDate = Carbon::parse($subscriptions->trial_end_date)->format('Y-m-d');
                    $toNumber = $business_location->mobile;
//                    $body = "تم تفعيل الاشتراك بنجاح ({$package->name}) بداية الاشتراك  ({$start_date} ) يمكن الدخول عبر https://mhamcloud.com";
                    $body = "تم تفعيل الاشتراك بنجاح ({$subscriptions->package->name}) بداية الاشتراك  ({$startDate}) الدخول عبر https://mhamcloud.com";
                    $this->SendSMS($toNumber, $body);
                }elseif($business_location->mobile == null || $validateNumber == false){
                    $output = ['warning' => 1,
                        'msg' => __('lang_v1.mobile_number_is_not_valid')
                    ];
                    return $output;
                }else{
                    $output = ['success' => 0,
                        'msg' => __('lang_v1.date_is_not_valid')
                    ];
                    return $output;
                }
...
// Further processing ...
    ...
    public function editSubscription($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('superadmin::'.viewSource().'superadmin_subscription.edit_date_modal')
